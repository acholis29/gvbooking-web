...
const SkeletonDetailProduk: React.FC<SkeletonDetailProdukProps> = ({ id }) => {
  return (
    <div
      role="status"
    ...
    >
      ...
      {/* Mini banner */}
      <div className="max-w-screen-xl mx-auto px-4 pb-4 overflow-x-auto md:overflow-visible">
        <div className="flex md:grid md:grid-cols-5 gap-4 min-w-max md:min-w-0">
          {[...Array(5)].map((_, i) => (
            <div
              key={i}
              className="flex-shrink-0 w-40 md:w-auto flex items-center justify-center h-30 mb-4 bg-gray-300 rounded-sm"
            >
              <svg
                className="w-10 h-10 text-gray-200"
                aria-hidden="true"
